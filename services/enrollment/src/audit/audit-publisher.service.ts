 ...
 * (enrollment created, enrollment dropped) to the shared audit
 * exchange, consumed by the Audit & Logging Service. Identical
 * implementation to identity's copy of this file — see that file for
 * the full rationale on the fire-and-forget tradeoff below.
 *
 ...
 * enrolling or dropping a course. The tradeoff (an audit gap during an
 * outage) is accepted here and should be called out explicitly in the
 * STRIDE threat model under Repudiation, with monitoring/alerting on
 * publish failures as the compensating control.
 */
@Injectable()
